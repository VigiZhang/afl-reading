...
static void add_instrumentation(void) {
  ...
  while (fgets(line, MAX_LINE, inf)) {
	...
	/* [analysis] 5. 不满足上述条件的代码不进行插桩，直接写入outf */
    fputs(line, outf);
	...
	*/
	...
	/* [analysis] 9. 不对特定的__asm__块插桩，设置skip_app */
    if (line[0] == '#' || line[1] == '#') {
    ...
    /* Label of some sort. This may be a branch destination, but we need to
       ...
       conventions. */
	/* [analysis] 11. 此处设置延迟插桩，用于跳转目的地址和函数开头
					跳转的目的label和函数label包含冒号:，设置instrument_next = 1
					在下次循环开头判断处插桩
	*/
#ifdef __APPLE__
  ...
  /* [analysis] 13. 这里会插入插桩代码所用的函数定义，其中包括：
  					__afl_maybe_log
  					__afl_store
  					__afl_return
  					__afl_setup
  					...
  					__afl_die
  					__afl_setup_abort
  					只当前面过程中存在插桩，即ins_lines不为0时才会进行
  */
  if (ins_lines)
...
int main(int argc, char** argv) {
  ...
  /* When compiling with ASAN, we don't have a particularly elegant way to skip
     ...
     that... */
  /* [analysis] 3. 使用ASAN或MSAN将inst_ratio除以3，此trick用以减少ASAN代码的分支插桩 */
