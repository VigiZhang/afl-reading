...
static void read_testcases(void) {
  ...
  /* [analysis] 40. 存在in_dir/queue则以in_dir/queue作为in_dir */
...
static void maybe_add_auto(u8* mem, u32 len) {
  ...
  /* [analysis] 52. 添加新字典项到自动生成字典a_extras */
...
EXP_ST void init_forkserver(char** argv) {
  ...
  /* [analysis] 90. fork server创建失败，打印原因 */
  if (child_timed_out)
...
static u8 run_target(char** argv, u32 timeout) {
  ...
  if (dumb_mode == 1 || no_forkserver) {
    ...
    if (!child_pid) {
      ...
      execv(target_path, argv);	/* [analysis] 92. dumb mode或no forkserver直接执行 */
  ...
  /* [analysis] 97. 分类报告退出原因FAULT_TMOUT | FAULT_CRASH | FAULT_ERROR | FAULT_NONE */
...
abort_calibration:
  ...
  /* [analysis] 101. 新路径发现标记，可变字节标记，显示状态界面等 */
...
static void perform_dry_run(char** argv) {
  ...
  while (q) {
	...
	/* [analysis] 102. 根据test case执行状态显示输出 */
    switch (res) {
...
static void maybe_delete_out_dir(void) {
  ...
  if (f) {
	...
	/* [analysis] 24. 如果out_dir/fuzzer_stats中显示上次fuzz运行时间大于25分钟
					则提示换目录或采用 -i - resume模式
	*/
  ...
  if (in_place_resume) {
	/* [analysis] 25. 在resume模式中，尝试将out_dir/queue重命名为out_dir/_resume使用
					如果out_dir/_resume存在，则说明目前的out_dir/queue并不完整
					将继续使用out_dir/_resume，并忽略重命名错误
	*/
  ...
  /* [analysis] 26. 非resume模式，首先删除out_dir/.synced */
  if (!in_place_resume) {
  ...
  /* [analysis] 27. 依次删除.state目录文件及目录：
  					out_dir/queue/.state/deterministic_done/id[:_]*
  					out_dir/queue/.state/auto_extras/auto_*
  					out_dir/queue/.state/redundant_edges/id[:_]*
  					out_dir/queue/.state/variable_behavior/id[:_]*
  					out_dir/queue/.state
  */
  ...
  if (rmdir(fn) && errno != ENOENT) goto dir_cleanup_failed;
  ck_free(fn);
  /* [analysis] 28. 删除out_dir/queue/id[:_]* */
  ...
  if (!in_place_resume) {
	/* [analysis] 29. 非resume模式下，尝试删除out_dir/crashes/README.txt */
  ...
  /* [analysis] 30. 在resume模式下，如果out_dir/crashes不为空则将其备份 */
  ...
  /* [analysis] 31. 删除out_dir/crashes/id[:_]* */
  if (delete_files(fn, CASE_PREFIX)) goto dir_cleanup_failed;
  ck_free(fn);
  ...
  /* Backup hangs, too. */
  /* [analysis] 32. 同样，out_dir/hangs文件不为空也备份起来 */
  if (in_place_resume && rmdir(fn)) {
  ...
  /* [analysis] 33. 删除out_dir/hangs/id[:_]* */
  if (delete_files(fn, CASE_PREFIX)) goto dir_cleanup_failed;
  ck_free(fn);
...
static u8 trim_case(char** argv, struct queue_entry* q, u8* in_buf) {
  /* [analysis] technical_details.txt 5) Trimming input files:
  				尝试按序使用power-of-two递增的长度删除块运行并观察是否有影响到覆盖率
  				如无影响则保存trim后的case
  */
...
static u8 fuzz_one(char** argv) {
  ...
  /*[analysis] 123. bitflip 按位翻转变异，依次进行：
...
#define EFF_SPAN_ALEN(_p, _l) (EFF_APOS((_p) + (_l) - 1) - EFF_APOS(_p) + 1)
  ...
  for (stage_cur = 0; stage_cur < stage_max; stage_cur++) {
	...
	*/
    if (!eff_map[EFF_APOS(stage_cur)]) {
	  ...
	  /* [analysis] 126. 文件大小小于128字节则都标记为“有效” */
  ...
  /* If the effector map is more than EFF_MAX_PERC dense, just flag the
     ...
     anyway. */
  /* [analysis] 127. 文件中90%以上的bytes都是“有效”的话，将全文件bytes标记为“有效” */
...
skip_bitflip:
  ...
  /* [analysis] 128. arithmetic 算术运算变异，依次进行：
...
skip_arith:
  ...
  /* [analysis] 130. interest 替换使用预设边界值和特殊值，依次进行：
...
skip_interest:
  ...
  /* [analysis] 131. dictionary 使用用户和自动生成字典替换(over)和插入(insert)，依次进行：
  					user extras (over)
  					user extras (insert)
  					auto extras (over)
  */
...
skip_extras:
  ...
  /* [analysis] 136. 到此，deterministic变异完毕 */
  ...
  /* [analysis] 137. havoc 随机混乱大变异，随机选取一种变异方式，进行变异fuzz
  					如果指定字典，有17种；没有指定字典，则有15种：
  					 0 => 随机翻转1bit
...
havoc_stage:
  ...
  /* [analysis] 139. splicing 剪接变异，这是一轮变异中的最后一项变异方式 */
...
static void sync_fuzzers(char** argv) {
  ...
  while ((sd_ent = readdir(sd))) {
	...
	/* [analysis] 110. 同步sync_dir/{d_name}/queue中的文件 */
	...
	/* [analysis] 111. 从out_dir/.synced/{d_name}中得到min_accept */
    ...
    while ((qd_ent = readdir(qd))) {
	  ...
	  /* [analysis] 112. 根据min_accept判断新的文件是否执行过 */
      if (qd_ent->d_name[0] == '.' ||
      ...
      if (st.st_size && st.st_size <= MAX_FILE) {
		...
		/* [analysis] 114. 保存unique crash或unique hang文件到out_dir
						如果新的路径，crash mode下，保存到queue
		*/
...
/* [analysis] 67. 检查ELF或Mach-O格式头Magic */
  ...
  /* [analysis] 68. 若不在qemu_mode或dumb_mode，则在内存中搜索__AFL_SHM_ID，判断是否插桩 */
  ...
  /* [analysis] 70. 检测是否使用ASAN */
  if (memmem(f_data, f_len, "libasan.so", 10) ||
      memmem(f_data, f_len, "__msan_init", 11)) uses_asan = 1;

  /* Detect persistent & deferred init signatures in the binary. */
  /* [analysis] 71. 检测persistent和deferred init签名 */
...
EXP_ST void setup_dirs_fds(void) {
  ...
  if (mkdir(out_dir, 0700)) {
	...
	/* [analysis] 23. 尝试创建out_dir，如已存在，则检测是否继续使用或是清理out_dir */
  ...
  */
  ...
  /* Gnuplot output file. */
  /* [analysis] 38. 创建out_dir/plot_data文件 */
...
int main(int argc, char** argv) {
  ...
  while ((opt = getopt(argc, argv, "+i:o:f:m:t:T:dnCB:S:M:x:Q")) > 0)
    ...
    switch (opt) {
      ...
      case 'i': /* input dir */
		/* [analysis] 2. -i指定输入目录in_dir
						当指定 -i - 时，设置in_place_resume模式
		*/
      ...
      case 'M': { /* master sync ID */
		  ...
		  /* [analysis] 4. -M指定Master sync_id，且强制执行确定性变异fuzz */
      ...
      case 'd': /* skip deterministic */
		/* [analysis] 5. -d选项跳过确定性变异过程，使用剪接变异方式 */
      ...
      case 'B': /* load bitmap */
		...
		/* [analysis] 6. -B选项读取之前fuzz产生的既有bitmap继续fuzz */
  ...
  /* [analysis] 10. 检查ASAN选项
  					ASAN_OPTIONS必须包含abort_on_error=1:symbolize=0
  					MSAN_OPTIONS同理
  */
  check_asan_opts();

  /* [analysis] 11. 处理-M和-S选项
  					包括选项检查，sync_id检查，更改sync_dir和out_dir
  */
  ...
  */
  ...
  /* [analysis] 22. 创建所需输出文件夹 */
  setup_dirs_fds();
  /* [analysis] 39. 从输入文件夹中读取testcase，存入queue中 */
  read_testcases();
  ...
  while (1) {
    ...
    if (!queue_cur) {
	  ...
	  /* [analysis] 108. 如果一次循环未发现新的路径，尝试使用剪接策略 */
